...
{
	auto ParameterSlider = [this, &dirty](const char* label, int& value, int min, int max)
	{
...
void Terrain::Tick(float deltaTime)
{
	...
	for (int z = 0; z < 256; z++)
	{
		for (int y = 0; y < 256; y++)
		{
			...
			{
				Plot(x, y, z, 0);
				Plot(x, (noise.GetNoise((float)x, (float)z) + 1.0f) * 8, z, WHITE);

				//noiseData[index++] = (noise.GetNoise((float)x, (float)y) + 1.0f) / 2.0f;
			}
		}
	}
