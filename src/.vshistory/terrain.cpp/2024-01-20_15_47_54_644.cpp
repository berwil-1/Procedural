...
{
	auto ParameterSlider = [](const char* label, int& value, int min, int max)
	{
		...
		if (ImGui::Button("Random"))
			value = (rand() ^ (rand() << 15) ^ (rand() << 30)) % (INT_MAX / 2);

		ImGui::PopStyleColor(3);
		ImGui::SameLine();
		ImGui::SliderInt(label, &value, -INT_MIN / 2, INT_MAX / 2);
	};
	...
	ImGui::PushStyleColor(ImGuiCol_ButtonHovered, ImColor::HSV(0.3f, 0.7f, 0.7f).Value);
	ImGui::PushStyleColor(ImGuiCol_ButtonActive, ImColor::HSV(0.3f, 0.8f, 0.8f).Value);
	if (ImGui::Button("Random")) seed = (rand() ^ (rand() << 15) ^ (rand() << 30)) % (INT_MAX / 2);
	ImGui::PopStyleColor(3);
	ImGui::SameLine();
