...
void Terrain::HandleInterface()
{
	...
	const char* items[] = { "Biome", "Evaluation", "Temperature", "Humidity", "Wind", "Precipitation" };
	static int item = 0;
	ImGui::Combo("Preset", &item, items, IM_ARRAYSIZE(items));
	...
	ImGui::PushStyleColor(ImGuiCol_Button, ImColor::HSV(0.2f, 0.6f, 0.6f).Value);
	ImGui::PushStyleColor(ImGuiCol_ButtonHovered, ImColor::HSV(0.2f, 0.7f, 0.7f).Value);
	ImGui::PushStyleColor(ImGuiCol_ButtonActive, ImColor::HSV(0.2f, 0.8f, 0.8f).Value);
	ImGui::Button("Random");
