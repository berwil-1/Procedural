...
void Terrain::Tick(float deltaTime)
{
	...
	if (dirty)
	{
		...
		const Biome& biome = BiomeFunction(0.2, 0.3, 0.5);
		...
		/*for (int z = 0; z < 512; z++)
		{
			for (int y = 0; y < 16; y++)
			{
				for (int x = 0; x < 512; x++)
				{
					float n = (noise.GetNoise((float)x, (float)z) + 1.0f) * 8.0f;

					if (y < n)
					{
						if (rand() % 10 < y && y > n - 2)
							Plot(x, y, z, PALETTE_SNOW[rand() % 8]);
						else if(y > n - (rand() % 4) - 4)
							Plot(x, y, z, PALETTE_DIRT_STONE[rand() % 8]);
						else
							Plot(x, y, z, PALETTE_STONE[rand() % 8]);
					}
					else if(y < 10)
						Plot(x, y, z, PALETTE_WATER[rand() % 8]);
					else
						Plot(x, y, z, 0);
				}
			}
