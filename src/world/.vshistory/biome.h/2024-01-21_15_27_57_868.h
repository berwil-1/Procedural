...
constexpr float cabs(const double x)
{
...
struct Biome
{
	...
	(
		...
		elevationLength(abs(elevationMax - elevationMin)),
		temperatureLength(abs(temperatureMax - temperatureMin)),
		humidityLength(abs(humidityMax - humidityMin)),
		surface(surface),
		layers(layers),
		liquids(liquids)
	{
